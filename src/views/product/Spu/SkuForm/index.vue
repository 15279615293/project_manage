      ...
      <el-form-item label="图片列表">
        <el-table style="width: 100%" border :data="spuImageList" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="prop" label="操作" width="width">
            <template slot-scope="{row}">
              <el-button type="primary" v-if="row.isDefault==0" @click="changeDefaut(row)">设为默认</el-button>
...
export default {
  ...
  methods: {
    ...
    async getData(category1Id, category2Id, spu) {
      ...
      //获取销售属性
      let result1 = await this.$API.spu.reqSpuSalaAttrList(spu.id);
      if (result1.code == 200) {
    ...
    async save(){
      ...
      let result = await this.$API.spu.requAddSku(skuInfo)
      if(result.code==200){
